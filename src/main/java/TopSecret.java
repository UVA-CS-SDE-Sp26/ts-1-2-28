...
public class TopSecret {
    public static void main(String[] args) {
        ...
        if (args.length == 0) { // no arguments = list the available files
            ...
            ProgramControl.runOneArgument(fileNumber); // display file with default key
            return;
        }
        else if (args.length == 2) { // 2 arguments = show file with custom key
            String fileNumber = args[0];
            String decipherKey = args[1];

            boolean validNumber = fileNumber.matches("\\d{2}");
            boolean validFile = decipherKey.endsWith(".txt");
            ...
            if (!validNumber && !validFile) {
                ...
                ProgramControl.runTwoArguments(fileNumber, new File(decipherKey));
            }
            ...
            System.out.println(
                    "error: too many arguments. options are:\n" +
                            "java TopSecret\n" +
